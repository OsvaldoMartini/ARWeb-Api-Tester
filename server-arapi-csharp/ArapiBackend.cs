...
public sealed class ArapiBackend
{
  ...
  public IResult ExportBotJobBash(string id, string? environmentId)
  {
    lock (_gate)
    {
      var job = _state.BotJobs.FirstOrDefault(item => string.Equals(item.Id, id, StringComparison.OrdinalIgnoreCase));
      if (job is null)
      {
        return Results.NotFound(new { error = "BotJob not found" });
      }

      var selectedEnvironmentId = string.IsNullOrWhiteSpace(environmentId)
        ? _state.Environments.FirstOrDefault(item => item.IsDefault)?.Id ?? "mock"
        : environmentId;
      var environment = _state.Environments.FirstOrDefault(item =>
        string.Equals(item.Id, selectedEnvironmentId, StringComparison.OrdinalIgnoreCase));
      if (environment is null)
      {
        return Results.BadRequest(new { error = $"Environment \"{selectedEnvironmentId}\" not found" });
      }

      var blocks = _state.BotJobBlocks.Where(block => block.BotJobId == id).OrderBy(block => block.Order).ToArray();
      var blockOrder = blocks.Select((block, index) => new { block.Id, Index = index })
        .ToDictionary(item => item.Id, item => item.Index);
      var commands = _state.BotJobCommands
        .Where(command => blockOrder.ContainsKey(command.BlockId))
        .OrderBy(command => blockOrder[command.BlockId])
        .ThenBy(command => command.Order)
        .ToArray();
      var variables = _state.BotVariables.Where(variable => variable.BotJobId == id).OrderBy(variable => variable.Name).ToArray();
      var endpoints = _state.ApiEndpoints.ToDictionary(endpoint => endpoint.Id, StringComparer.OrdinalIgnoreCase);

      try
      {
        var export = BotJobBashExporter.Generate(job, environment, commands, variables, endpoints);
