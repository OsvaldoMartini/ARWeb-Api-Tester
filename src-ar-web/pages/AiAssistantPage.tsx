    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="relative w-full max-w-2xl rounded-lg border border-border bg-surface shadow-2xl">
        <div className="flex items-start justify-between border-b border-border px-6 py-4">
          ...
          <button className="ml-4 shrink-0 rounded p-1 text-text-muted hover:bg-surface-alt hover:text-text" onClick={onClose} title="Close without changing mode">
            <X size={16} />
          </button>
        </div>
        <div className="space-y-4 px-6 py-4">
          <p className="text-sm leading-relaxed text-text-muted">
            Simulate banking conversations either from a bank employee or from an end client through the e-banking channel.
          </p>
        ...
        <div className="flex gap-3 border-t border-border px-6 py-4">
          <button
            className={`flex flex-1 items-center justify-center gap-2 rounded-md border px-4 py-2.5 text-sm font-medium transition-colors ${currentMode === 'employee' ? 'border-primary bg-primary text-white' : 'border-border bg-surface-alt hover:bg-surface hover:border-primary hover:text-primary'}`}
            onClick={() => onSelect('employee')}
          >
            👔 Start as Bank Employee
          </button>
          <button
            className={`flex flex-1 items-center justify-center gap-2 rounded-md border px-4 py-2.5 text-sm font-medium transition-colors ${currentMode === 'client' ? 'border-primary bg-primary text-white' : 'border-border bg-surface-alt hover:bg-surface hover:border-primary hover:text-primary'}`}
...
export function AiAssistantPage() {
  ...
  useEffect(() => {
    if (!localStorage.getItem(INTRO_SEEN_KEY)) setShowWelcome(true);
    ...
    <>
      {showWelcome && (
        <WelcomeModal
          ...
          onClose={() => { localStorage.setItem(INTRO_SEEN_KEY, '1'); setShowWelcome(false); }}
        />
      )}
      <div className="flex h-full flex-col">
        <PageHeader
          ...
          actions={
            <div className="flex items-center gap-2">
              <button className="btn text-xs gap-1" onClick={() => setShowWelcome(true)} title="Switch conversation mode">
                <Info size={14} /> Mode
              </button>
              <button className="btn text-xs" onClick={() => setShowCaps((v) => !v)}>Coverage</button>
              <select className="input w-52" value={agentId} onChange={(e) => setAgentId(e.target.value)}>
                <option value="">Auto-route</option>
                {agents.map((a) => (
                  <option key={a.id} value={a.id}>{a.name}{a.capabilityCount > 0 ? ` (${a.capabilityCount})` : ' ○'}</option>
                ))}
              </select>
            </div>
          }
        ...
        <div className="card flex min-h-0 flex-1 flex-col">
          <div className="min-h-0 flex-1 space-y-3 overflow-y-auto pr-1">
            {turns.length === 0 ? (
              <p className="text-sm text-text-muted">
                {mode === 'employee' ? 'Ask something like "Show me the full customer 360 profile for client Mario Rossi".' : 'Ask something like "What is the current balance of my accounts?"'}
              </p>
            ) : (
              turns.map((t, i) =>
                t.role === 'user' ? (
                  <div key={i} className="flex justify-end">
                    <div className="max-w-[80%] rounded-md bg-primary px-3 py-2 text-sm text-white">{t.text}</div>
                  </div>
                ) : (
          ...
          <div className="mt-3 flex gap-2">
            <input
              className="input"
              placeholder={mode === 'employee' ? 'Ask as a bank employee…' : 'Ask as an e-banking client…'}
              value={question}
