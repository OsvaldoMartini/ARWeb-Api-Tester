...
export function SeparationProgressPage() {
  ...
  return (
    <div>
      ...
      {error && (
        <div className="card mb-4 text-sm" style={{ borderColor: 'var(--color-danger)', color: 'var(--color-danger)' }}>
          Could not load progress.json: {error}
        </div>
      )}
            ...
            {data.phases.map((phase) => (
              <div key={phase.id} className="card flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  ...
                  {phase.note && (
                    <div className="text-xs text-text-muted mt-1">{phase.note}</div>
                  )}
                </div>
                <span className={`badge ${STATUS_CLASS[phase.status]} shrink-0`}>
